	...
	*/
	...
	public function actionGetNotification(){
		...
		if(!isset($apiData['data']['token']) && empty($apiData['data']['token'])){
			...
			$userId =$apiData['data']['user_id'];
			$checkUserData = User::model()->find("id='".$apiData['data']['user_id']."' AND token = '".$apiData['data']['token']."'");
			if($checkUserData){
				$connection=Yii::app()->db;
				...
				foreach($activityDatas as $activityData){
					...
					$res['activity_detail_id'] = $activityData['activity_detail_id']?$activityData['activity_detail_id']:'';
					...
					if($activityData['feed_id']!=0)
					{
						$PostData = PostJob::model()->find("post_id ='".$activityData['feed_id']."' ");
						$res['post_id'] = $activityData['feed_id'];

						$res['job_title'] = $PostData->job_title. "";
                        $res['job_description'] = $PostData->job_description. "";
                        $res['category_id'] = $PostData->category_id. "";
                        $res['subcategory_id'] = $PostData->subcategory_id. "";

                        $res['category_name'] = $this->GetCategoryName($PostData->category_id);
                        $res['subcategory_name'] = $this->GetSubCategoryName($PostData->subcategory_id);
					...
					else
					{
                        ...
                        $res['category_name'] = 0;
                        $res['subcategory_name'] = 0;

					}
					
					
					$res['datetime'] = $activityData['created_at'];
	...
	public function actionCountUnreadNotifications(){
		$res = array();
		$response=array();
		$friend_feed=array();
		$apiData = json_decode(file_get_contents('php://input'),TRUE);
